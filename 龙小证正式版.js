...
if (parseInt(device.sdkInt) < 24) {
  alert("您手机安卓版本低于7.0版本，无法使用该程序。非华为手机无法保证完美支持！");
  exit();
}
...
if (!(typeof(check_f[0]) == "undefined"))
  var d_name_def = check_f[0].replace(".txt", "");
else
  var d_name_def = "";
...
function xvxy() {
  ...
  console.log(n_v);
  var path_x = context.getFilesDir() + "/" + d_name + ".txt";
  var file_x = open(path_x, "r");
  user_data = file_x.readlines();
  file_x.close();
  if (user_data[4] == 0) {
    user_data[4] = user_data[6] = user_data[8] = user_data[10] = p_1y = p_2y = p_3y = p_4y = String(n_v);
    var file_w = open(path_x, "w");
    file_w.writelines(user_data);
    file_w.close();
    var file_y = open(path_x, "r");
    user_data = file_y.readlines();
    file_y.close();
    console.show();
    for (var i = 0; i < 13; i++) {
...
function jjz2() {
  ...
  sub_score = sub_score + 10;
  sleep(1600);
  console.error("申请过程结束,请点击音量+按键，结束龙小证!，如果不点的话，无法停止哦！哦，对，他是一匹马，不让它停，它是停不下来的。哈哈哈");
}

console.hide();
var d_name = "";
var path = context.getFilesDir() + "/" + d_name_def + ".txt";
var path_a = context.getFilesDir();
var user_data = new Array();
var user_adata = new Array();
if (files.exists(path) && (confirm("申请人" + d_name_def + "\n" + "信息是否正确?"))) {
  ...
  d_name = rawInput("请输入驾驶人名字", d_name_def);
  ...
  var path_w = context.getFilesDir() + "/" + d_name + ".txt";
...
function abstime() {
  ...
  var q_data = String(parseInt(w_data) + 1);
  d_time = rawInput("设置脚本运行时间！" + "\n" + "\n" + "若即刻申请，直接点击确定;" + "\n" + "\n" + "若定时申请，将默认时间修改到定时时间，再点击确定！", qqTime);
  d_ap = rawInput("请输入需要申请哪天的进京证;注意：进京证申请只能是未来五天的某一天，更长时间是不能申请的！！输入内容实例：比如今天是" + w_data + "号，那么请输入：" + q_data, q_data);
