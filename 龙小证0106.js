...
threads.start(function () {
  events.on("exit", function () {
    console.hide();
    device.vibrate(70);
    alert("《龙小证》结束运行!如果是异常退出，请记得截图，发到优化群组方便分析，同时请附带以下场景的描述。");
...
if (parseInt(device.sdkInt) > 23)
  ...
  alert("安卓7.0版本以上可实现一键申请，低于安卓7.0程序只能自动申请到上传图片阶段！");
      ...
      if (textOrDesc("车辆及进京证信息").exists()) {
        if (t_text_a.indexOf("申请") > 0) {
          if ((textContains("有效期").exists() || descContains("有效期").exists()) && ((textOrDesc("查看").exists()) || textOrDesc("审核中").exists()) && (!textOrDesc("+申请").exists())) {
            console.error("刚刚申请已经成功或您之前申请的进京证还没有到期，请到期后再来申请吧！");
            home();
            exit();
          }
        }
...
function abstime() {
  sub_score++;
  var i = 0;
  d_time = rawInput("开始申请过程的时间,要马上申请的话可以直接输入：马上，定时申请的话，要符合如下格式;如2017-11-02T21:30:00", "马上");
  d_ap = rawInput("请输入需要申请哪天的进京证;注意：进京证申请只能是未来五天的某一天，更长时间是不能申请的！！比如今天是12号最晚只能申请17号！！输入内容实例：比如您想申请14号的，那么请输入：14");
